...
| `blocks data schema list/pull/push`, `blocks data rules pull/deploy`, `blocks data sync` | Author and deploy Data Gateway schemas (`MentorProfile`, `MenteeProfile`, `AvailabilityRule`, `Session`, `Review`, `Report`, `AuditTrail`) and their access rules | Backs FR-6–24, FR-30–32, FR-35–37 (§02-REQUIREMENTS.md) |
| `blocks data files upload` / `dms-upload` | Presigned upload + DMS registration for files | Backs FR-6 (mentor profile photo) |
| `blocks auth users *`, `blocks auth roles/permissions *`, `blocks auth mfa *`, `blocks auth oidc-clients *` | IAM admin: users (multi-role capable — a user can carry `mentee` + `mentor` simultaneously), MFA, OIDC client registration | Backs FR-1–5, FR-35, FR-30 (user list/suspend done directly in Blocks' console per FR-38, not via a custom CLI-scripted UI) |
| IAM Signup Settings (`isSSoSignUpEnabled`, confirmed present in `src/commands/iam/signup-settings/save.ts`) + Blocks Cloud console **External IdP** config (JWKS URL + issuer + claim mapping, per the `blocks-external-idp` skill in this workspace) | Federates an upstream identity provider by JWKS, not a hardcoded "Google button" — the skill confirms Google's JWKS (`googleapis.com/oauth2/v3/certs`) passes Blocks' CORS check; LinkedIn's was never tested | Backs FR-1 for Google only. LinkedIn is explicitly **not** backed yet — a real, unverified gap, not an oversight (`01-BRD.md` §8 v1.1) |
