# MentorMatch (AdpList Clone)

## Project layout

```
adplist-clone/
  web/                 Next.js 16 App Router app (RSC-first)
  blocks/              Blocks CLI workspace (schemas, rules, IAM seed)
  .codex/skills/       Vendored SELISE Blocks agent skills
  .claude/skills/      Claude pointer stubs → .codex/skills
  docs/                BRD, architecture, design system, specs, plans
  .understand-anything/domain-graph.json
```

## Blocks project

- **Key:** `D67e7edaf9e1d432288361305fcc74c47`
- **Name:** AdpList
- **App domain:** https://dpkhbr.slsblx.com
- **OIDC client:** `d85f2f64-6dab-4052-8278-99e3adf6e67a`

## Next.js agent rules

Read bundled docs at `web/node_modules/next/dist/docs/` before writing Next.js code. The managed block in `web/AGENTS.md` is auto-maintained by `next dev`.

## Phase 1 scope (current)

Login (Blocks OIDC), IAM roles (admin/mentor/mentee), profile schemas, mentorship assignments, seeded users, dashboard (mentor sees assigned mentees read-only) + profile update. No booking/directory/video yet.

## Seed accounts

| Email | Password | Role |
|---|---|---|
| noor@yopmail.com | Pass@123 | admin |
| mentor1@yopmail.com | Pass@123 | mentor |
| mentor2@yopmail.com | Pass@123 | mentor |
| mentor3@yopmail.com | Pass@123 | mentor |
| mentor4@yopmail.com | Pass@123 | mentor |
| mentor5@yopmail.com | Pass@123 | mentor |
| mentee1@yopmail.com … mentee10@yopmail.com | Pass@123 | mentee |

Seed data: 5 mentors, 10 mentees, each mentee assigned to all 5 mentors (50 `MentorshipAssignment` records). Run `node scripts/seed-platform-data.mjs` after `blocks data sync`.

Example: log in as `mentor1@yopmail.com` (Adnan Bhai, design) → dashboard shows skill-matched mentees such as Yasmine Khan and Nadia Islam (read-only), not engineering mentees like Zain Ali.
...
OIDC session cookies require HTTPS locally — see `.codex/skills/blocks-frontend-local-https/SKILL.md`.
