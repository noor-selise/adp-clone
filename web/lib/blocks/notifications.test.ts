...
test("notifyUser delivers to the target user's own inbox (AC-1, AC-7)", async () => {
...
test('notifyRole delivers to every member of the role (AC-2, AC-7)', async () => {
...
test('notifyRole with no current members completes without throwing (AC-5)', async () => {
...
test('notifyUser with no matching account completes without throwing (AC-5)', async () => {
  await assert.doesNotReject(() =>
...
test('notifyUser swallows a notifier failure and never throws (AC-4)', async () => {
  resetServiceBlocksClientForTest()
